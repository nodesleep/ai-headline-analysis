...
<template>
  <div class="flex min-h-screen w-full 2xl:w-[80rem] mx-auto">
    <div class="flex flex-grow p-6 gap-3">
      ...
      <div class="w-2/3">
        <div class="flex">
          <h1 class="font-bold text-xl uppercase" v-if="!queryStore.loading">Analysis</h1>
          <div v-if="queryStore.loading" class="px-3 py-1 mt-6 bg-emerald-600 animate-pulse text-white text-sm">
            <p>Analyzing headline</p>
          </div>
        </div>
        <div v-if="queryStore.analysis" class="flex flex-col font-bold text-xs gap-3">
          ...
          <div class="flex flex-col p-2 border-2 border-black">
          <h1 class="font-bold text-lg">Sentiment is {{ queryStore.analysis.sentiment }}</h1>
          <p>{{ queryStore.analysis.sentiment_based_content }}</p>
          </div>

          <div class="flex flex-col p-2 border-2 border-black">
          <h1 class="font-bold text-lg">Perspectives</h1>
          <p class="mb-3">Positive: {{ queryStore.analysis.positive_perspective }}</p>
          ...
          <p>{{ queryStore.analysis.interactive_scenarios.scenario }} {{ queryStore.analysis.interactive_scenarios.outcome }}</p>
            </div>
            <div class="flex flex-col p-2 border-2 border-black">
          <h1 class="font-bold text-lg">Conclusion</h1>
          <p class="mb-3">{{ queryStore.analysis.conclusion }}</p>
          <p>{{ queryStore.analysis.question_answer.question }} {{ queryStore.analysis.question_answer.answer }}</p>
          </div>
        </div>
